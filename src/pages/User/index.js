...
const User = ({
  ...
  useEffect(() => {
    if (rowEdited) {
      ...
      setValue("idHeadquarters", rowEdited.idHeadquarters);
    }
  }, [rowEdited]);
              ...
              <div className="row">
                <div className="col s12 m6">
                  <div className="form-group">
                    ...
                    <select
                      {...register("idHeadquarters")}
                      id="idHeadquarters"
                      className={`custom-select ${
                        errors.idHeadquarters && "is-invalid"
                      }`}
                    >
                      <option value={""}>Seleccionar…</option>
                      {headquartersses &&
                        Object.keys(headquartersses).length > 0 &&
                        headquartersses.items.map((ele, key) => (
                          <option key={key} value={ele.idHeadquarters}>
                            {ele.nameHeadquarters}
                          </option>
                        ))}
                    </select>
                  </div>
...
const mapStateToProps = (state) => {
  return {
    ...
    headquartersses: state.headquartersState.headquartersses,
  };
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    reduxGetHeadquartersses: (payload) =>
      dispatch({
        type: "FETCH_HEADQUARTERSSES_REQUEST",
        value: payload,
      }),
