          ...
          <thead>
            <tr>
              {cols.map((headerItem, index) => (
                <th className="text-nowrap" key={index}>{(typeof headerItem.title == 'string') ? headerItem.title : headerItem.title()}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {data.length > 0 ? (
              data.map((item, index) => (
                <tr key={index}>
                  {cols.map((col, key) => (
                    <td className="text-nowrap" key={key}>{col.render(item)}</td>
                  ))}
                </tr>
              ))
            ) : (
